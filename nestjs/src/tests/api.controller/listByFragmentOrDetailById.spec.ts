...
describe('listByFragmentOrDetailById()', () => {
  it('should list all data associated to the fragment', async () => {
    ...
    // when
    const result = await restController.listByFragmentOrDetailById(
      fragment,
      reqMock,
    );
  ...
  it('should return detail if unknown fragment', async () => {
    ...
    // when
    const result = await restController.listByFragmentOrDetailById(id, reqMock);
  ...
  it('should return 404 if an unknown fragment/id', async () => {
    ...
    // when
    try {
      await restController.listByFragmentOrDetailById(id, reqMock);
    } catch (ex) {
      // then
